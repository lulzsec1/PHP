<?php
$id = $_GET["id"]; ## BURADA GELEN İD'ye göre sıralama yapması için  yakalama noktası koyduk. 
					## BURASI Blog.php'deki kategori bölmesine aittir.


		$konu 	= $db->prepare("select * from konular inner join kategoriler on 
		kategoriler.kategori_id = konular.konu_kategori where konu_kategori=?"); #databaseten konular tablomuza bağlanıyoruz. İnner join ile alt içeriğe bağlanıyoruz. Daha sonra where ile kategorilerimize bağlantı kuruyoruz.
...
<?php  echo mb_substr($m["konu_aciklama"],0,475);# BURADA DATABASEDEN GİRİLMİŞ OLAN KONU AÇIKLAMALARII ALIYORUZ
	...
	<?php 
			...
			echo '<div class="hata"> Bu Kategoriye Ait Hiçbir Konu Bulunmamaktadır...</div>';
		}
	?>